...
class MultiSampleImageLogger(ImageLogger):
    def __init__(self, num_samples_per_condition=8, **kwargs):
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if (
            ...
            with torch.no_grad():
                ...
                for sample_idx in range(self.num_samples_per_condition):
                    
                    current_seed = pl_module.global_step * 1000 + sample_idx
                ...
                final_images = {}
                for key, img_list in all_samples.items():
                   
                    final_images[key] = torch.cat(img_list, dim=0)

            
            for k in final_images:
                N = min(final_images[k].shape[0], self.max_images)
                if not isheatmap(final_images[k]):
                    final_images[k] = final_images[k][:N]
                if isinstance(final_images[k], torch.Tensor):
                    final_images[k] = final_images[k].detach().float().cpu()
                    if self.clamp and not isheatmap(final_images[k]):
                        final_images[k] = torch.clamp(final_images[k], -1.0, 1.0)
...
if __name__ == "__main__":
    ...
    try:
        ...
        pretrained_path = config.model.params.get('sync_path', None)
        ...
        if pretrained_path and os.path.exists(pretrained_path):
            ...
            if pretrained_path:
                print(f"Pretrained weights not found at {pretrained_path}. Starting training from scratch.")
            else:
                print(f"No pretrained weights path provided. Starting training from scratch.")
